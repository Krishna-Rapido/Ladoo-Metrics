...
/** Avoid stale JSON from browser disk cache in production (same-origin /knowledge/* GETs). */
const NO_CACHE_HEADERS = {
  "Cache-Control": "no-cache",
  Pragma: "no-cache",
} as const

async function authHeadersNoCache(userId?: string) {
...
export async function listSchemaTables(): Promise<SchemaTable[]> {
  const { data } = await axios.get(`${BASE_URL}/knowledge/tables`, {
    headers: await authHeadersNoCache(),
    timeout: TIMEOUT_MS,
  })
...
export async function listQueryHistory(
  ...
  const { data } = await axios.get(`${BASE_URL}/knowledge/queries`, {
    headers: await authHeadersNoCache(userId),
    timeout: TIMEOUT_MS,
  })
  return Array.isArray(data) ? data : []
}
